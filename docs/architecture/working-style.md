...
to work with the agent. The working style provides that continuity
...
persistent style cues and raw session history.
...
| Human collaboration preference | working style |
...
`style.md` is the only source of truth. It is created
owner-only (`0600`) by the pack helper when missing.

`.knot/style-pack.md` is a runtime snapshot. In direct scope it
...
`.knot/style.patch` is temporary agent output. It is applied only
through the deterministic helper.

## Style Contents

`style.md` records only durable collaboration cues:
...
The full rendered style file must stay under 1600 characters. Updates should merge
or replace older bullets rather than append indefinitely.

Structured style files may use only `version`, `updated`, and `reviewed`
...
most five bullets per section. Style files over 1200 characters should produce a
...
`style.md` must not absorb general enterprise knowledge or task notes. When in
doubt, route the information to the appropriate knowledge, planning, or SOP
path instead of stretching the style file.
...
- the style target must be exactly
  `workspace/users/<user_slug>/style.md`;
- all new runtime/style files are owner-only;
- patches must include a matching `base_sha256`;
- path traversal, absolute paths, symlinks, non-style targets, malformed
