...
If the backup root is not a git repository, or has no configured remote, do not
initialize git and do not add a remote. Report that backup setup is required and
include the missing condition.

If it is a git repository with a remote:
1. Check git status.
